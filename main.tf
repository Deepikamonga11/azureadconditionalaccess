...
resource "azuread_conditional_access_policy" "require_compliant_devices" {
  ...
  grant_controls {
    operator          = "AND"
    built_in_controls = ["compliantDevice"] # ✅ Enforce device compliance
  }
}
...
resource "azuread_conditional_access_policy" "require_intune_enrollment" {
  ...
  conditions {
    ...
    client_app_types = ["all"]  #  Covers all client app types
  ...
  grant_controls {
    operator          = "AND"
    built_in_controls = ["mfa", "compliantDevice", "compliantApplication"] #  Requires MFA, Compliant Device, Approved Apps
  }
}
resource "azuread_conditional_access_policy" "block_guests" {
  ...
  conditions {
    users {
      ...
      excluded_groups = var.admin_groups.guest_exclusion_group_block_guests
    }
...
resource "azuread_conditional_access_policy" "guest_application_access" {
  ...
  conditions {
    users {
      included_users = var.guest_user_types
      excluded_groups = var.admin_groups.guest_exclusion_group
    }
...
resource "azuread_conditional_access_policy" "usr_g_register_security_info" {
  ...
  conditions {
    users {
      included_users  = ["all"]
      excluded_groups = var.admin_groups.excluded_group_usr_g_register_security_info
    }
...
resource "azuread_conditional_access_policy" "usr_g_strong_authentication" {
  ...
  conditions {
    users {
      included_users  = ["all"]
      excluded_groups = var.admin_groups.excluded_group_usr_g_strong_auth 
    }
...
resource "azuread_conditional_access_policy" "usr_s_limit_user_sessions" {
  ...
  conditions {
    users {
      ...
      excluded_groups = var.admin_groups.excluded_group_usr_s
    }
    client_app_types = ["all"]
